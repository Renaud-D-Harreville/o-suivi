...
export const useEventStore = defineStore("event", () => {
  ...
  function buildCompetitorsFromData(data: { name: string; courses: Course[]; time_gates: TimeGates[]; competitors: any[] }): void {
    currentEventId.value = currentEventId.value;
  ...
  async function fetchTracking(eventId: string, force = false): Promise<boolean> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const cached = await eventCacheService.getTracking(eventId);
        if (cached) {
          currentEventId.value = eventId;
          buildCompetitorsFromData(cached as any);
        }
        return true;
      }
      ...
      currentEventId.value = eventId;
      buildCompetitorsFromData(data);

      // Persist to IndexedDB for offline access
      await eventCacheService.saveTracking(eventId, data);
